...
public enum PillEdge: String, Codable, CaseIterable { case top, bottom, left, right, free }
public enum PillGeometry {
    ...
    public static func center(edge: PillEdge, size: CGSize, in frame: CGRect, x: Double = 0.5, y: Double = 0.5) -> CGPoint {
        ...
        case .free: point = CGPoint(x: frame.minX + CGFloat(x) * frame.width, y: frame.minY + CGFloat(y) * frame.height)
        }
...
public final class PillPlacement {
    ...
    public func choice(for display: String) -> Choice? { saved.choices[display] }
