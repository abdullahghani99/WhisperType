...
## Polishing and correction learning follow-up

The latest-50 audit showed that the restored model was still being overruled by a word-level guard, losing punctuation with the rejected rewrite. Copyediting now has a dedicated policy, morphology-aware checks, negative-question and attribution checks, bounded stutter removal, and independently checked punctuation recovery. A second recovery step aligns punctuation onto the original word sequence, preserving identifiers. Numbered lists remain permitted. Prompt composition and meeting generation keep their existing model behavior.

The repaired policy was replayed against the 50 audited dictations. It produced 29 accepted edits, 8 punctuation recoveries and 13 unchanged results; none reached the verbatim-recovery path. Some recovery results legitimately equal the original and these counters are not accuracy scores. A final 20 reserved archive cases produced 14 accepted edits, 2 punctuation recoveries, 1 punctuation projection and 3 unchanged results. These cases are now consumed for future tuning. Earlier inspected batches were kept separate; final list/identifier safeguards were revalidated against recorded actual model generations. There are still awkward phrases and ambiguous sentence boundaries, so Wispr parity and a universal 10/10 remain unproven.

History and sent-item menus now provide Teach correction; meeting details provide task-specific note/transcript corrections. The active dictation model retrieves relevant explicit corrections immediately, and the Learning page displays saved feedback counts. Original outputs and recordings are retained. An isolated real-model check confirmed that a saved correction was retrieved and used; irrelevant corrections are excluded by separate tests.

The private learning cycle screened archive development pairs, excluded reserved inputs, and trained two staged 14B adapters with disjoint splits. The response-only experiment masks instruction tokens. Neither candidate demonstrated enough benefit to replace the base 14B model. Training and inference experiments retain exact corpus/policy/weight evidence outside public source. Meeting feedback remains separate, and ordinary outputs are never automatically treated as correct labels. See [Learning loop](learning-loop.md) for the operating procedure.
